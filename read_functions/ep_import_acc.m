%% LOTUS: Read and collate Accelerometer data
%  Copyright (C) (2024) Jack Fogarty

function [cfg,dat,summary] = ep_import_acc(cfg,d,s,subs,span)
...
% Identify the method used to collate output
    switch cfg.ReadMethod
        ...
        case 'Default'
            ...
            files  = dir(fullfile([cfg.parent_folder filesep cfg.day_folders(d).name filesep subs{s} filesep 'accelerometer*']));
            ...
            if ~isempty(tmp) & ~all(isnan(tmp),'all')
                ...
                tmp = [array2table(tmp(:,1),'VariableNames',{'unix_time'}) table(rtime,'VariableNames',{'local_time'}) array2table(tmp(:,2:4),'VariableNames',{'data_x','data_y','data_z'})];
                ...
                pts  = length(data.data_x);
                Pnan = sum(isnan(data.data_x))/length(data.data_x)*100; % Percent of selected period missing (nan)
                summary{1,1} = [array2table(dur,"VariableNames","ACC duration") array2table(pts,"VariableNames","ACC pts") array2table(Pnan,"VariableNames","ACC % Missing")];
                clear dur pts Pnan
            else
                data = [];
                summary{1,1} = {};
        ...
        case 'Custom'
            ...
            for d = 1:length(cfg.day_folders)
                tmp = dir(fullfile([cfg.day_folders(d).folder filesep cfg.day_folders(d).name filesep cfg.selected_subjects{s} filesep 'accelerometer*']));
                if isempty(files)
                   files = tmp;
                else
                   files = [files; tmp];
            ...
            if ~isempty(tmp)
                ...
                for e = 1:length(day_idx) % For each possible day (i.e., epoch/beginning points)
                    ...
                    pts  = length(dat{e,1}.data_x);
                    Pnan = sum(isnan(dat{e,1}.data_x))/length(dat{e,1}.data_x)*100; % Percent of selected period missing (nan)
                    summary{e,1} = [array2table(dur,"VariableNames","ACC duration") array2table(pts,"VariableNames","ACC pts") array2table(Pnan,"VariableNames","ACC % Missing")];
                    clear dur pts Pnan
                    else
                        summary{e,1} = {};
                    end
