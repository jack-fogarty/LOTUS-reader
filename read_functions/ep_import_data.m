            ...
            for d = 1:length({cfg.day_folders.name})
                ...
                for s = 1:length(subs)
                    ...
                    if cfg.DataType.EDA
                        ...
                        if ~isempty(summary)
                            if isempty(dat.Summary)
                                ...
                                dat.Summary{1,1} = horzcat(dat.Summary{1,:},summary{1,:}); % cellfun(@horzcat,dat.Summary,summary,UniformOutput=false);
                            end
                        end
                        clear summary
                    end
                    ...
                    if cfg.DataType.BVP
                        ...
                        [cfg,dat.BVP,summary] = ep_import_bvp(cfg,d,s,subs);
                    ...
                    if cfg.DataType.SystP
                        ...
                        [cfg,dat.SystP,summary] = ep_import_systp(cfg,d,s,subs);
                    ...
                    if cfg.DataType.Temp
                        ...
                        [cfg,dat.Temp,summary] = ep_import_temp(cfg,d,s,subs);
                    ...
                    if cfg.DataType.ACC
                        ...
                        [cfg,dat.ACC,summary] = ep_import_acc(cfg,d,s,subs);
...
%                         [cfg,dat.GYR] = ep_import_gyr(cfg,d,s,subs); 
%                     end
%                     
%                     if cfg.DataType.Steps
%                         % Collate the Steps data for this subject
%                         [cfg,dat.Steps] = ep_import_steps(cfg,d,s,subs); 
%                     end
%                     
...
%                     end                   
        ...
        case 'Custom'
                ...
                for s = 1:length(cfg.selected_subjects)
                    ...
                    % Collate the Tags data for this subject
                    [cfg,dat.Tags] = ep_import_tag(cfg,[],s,[],span);
                    
                    % Read in and collate the required EmbracePlus data
                    if cfg.DataType.EDA
                        % Collate the EDA data for this subject
                        [cfg,dat.EDA,summary] = ep_import_eda(cfg,[],s,[],span);
                    ...
                    if cfg.DataType.BVP
                        % Collate the BVP data for this subject
                        [cfg,dat.BVP,summary] = ep_import_bvp(cfg,[],s,[],span);
                    ...
                    if cfg.DataType.SystP
                        % Collate the Systolic Peak data for this subject
                        [cfg,dat.SystP,summary] = ep_import_systp(cfg,[],s,[],span);
                    ...
                    if cfg.DataType.Temp
                        % Collate the Temperature data for this subject
                        [cfg,dat.Temp,summary] = ep_import_temp(cfg,[],s,[],span);
                    ...
                    if cfg.DataType.ACC
                        % Collate the Accelerometer data for this subject
                        [cfg,dat.ACC,summary] = ep_import_acc(cfg,[],s,[],span);
...
%                     end
                    ...
                    % Return if dat structute is completely empty
                    if all(cellfun(@isempty,dat.Tags)) + sum(structfun(@isempty,dat)) == 10
                        % Reset dat
                        dat = pdat;
                        continue
                    end   
