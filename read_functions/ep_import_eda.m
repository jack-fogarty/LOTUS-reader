%% LOTUS: Read and collate EmbracePlus EDA data
%  Copyright (C) (2024) Jack Fogarty

function [cfg,dat,summary] = ep_import_eda(cfg,d,s,subs,span)
...
% Identify the method used to collate output
    switch cfg.ReadMethod
        ...
        case 'Default'
            ...
            files  = dir(fullfile([cfg.parent_folder filesep cfg.day_folders(d).name filesep subs{s} filesep 'eda*']));
            ...
            if ~isempty(tmp) & ~all(isnan(tmp),'all')
                ...
                summary{1,1} = [array2table(dur,"VariableNames","EDA duration") array2table(pts,"VariableNames","EDA pts") array2table(Pnan,"VariableNames","EDA % Missing")];
                clear dur pts Pnan
            else
                data = [];
                summary{1,1} = {};
        ...
        case 'Custom'
            ...
            for d = 1:length(cfg.day_folders)
                tmp = dir(fullfile([cfg.day_folders(d).folder filesep cfg.day_folders(d).name filesep cfg.selected_subjects{s} filesep 'eda*']));
                if isempty(files)
                   files = tmp;
                else
                   files = [files; tmp];
            ...
            if ~isempty(tmp)
                ...
                for e = 1:length(day_idx) % For each possible day (i.e., epoch/beginning points)
                    ...
                    if ~isempty(dat{e,1})
                        ...
                        summary{e,1} = [array2table(dur,"VariableNames","EDA duration") array2table(pts,"VariableNames","EDA pts") array2table(Pnan,"VariableNames","EDA % Missing")];
                        clear dur pts Pnan
                    else
                        summary{e,1} = {};
                    end
