...
% Identify the method used to collate output
    switch cfg.ReadMethod
        ...
        case 'Default'
            ...
            files  = dir(fullfile([cfg.parent_folder '\' cfg.day_folders(d).name '\' subs{s} '\eda*']));
        ...
        case 'Custom'
            ...
            for d = 1:length(cfg.day_folders)
                tmp = dir(fullfile([cfg.day_folders(d).folder '\' cfg.day_folders(d).name '\' cfg.selected_subjects{s} '\eda*']));
                if isempty(files)
                   files = tmp;
                else
                   files = [files; tmp];
